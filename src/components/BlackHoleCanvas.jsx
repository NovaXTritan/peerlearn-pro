...
const AccretionDiskMaterial = shaderMaterial(
  { uTime: 0, uInner: 0.36, uRing: 0.62, uThick: 0.22,
    uCore: new THREE.Color("#fff2c7").toArray(),
    uWarm: new THREE.Color("#ffb168").toArray(), uExposure: 1.6 },
  ...
  void main(){
    vec2 uv = rot(vUv, 0.10 * sin(uTime*0.22));
    float r = length(uv);
    float inner = smoothstep(uInner, uInner*0.96, r);
    float band = exp(-pow((r-uRing)/uThick, 2.0) * 2.0);
    float ang = atan(uv.y, uv.x);
    float beaming = 0.60 + 0.40 * cos(ang - uTime*0.40);
    band *= beaming;
    vec3 col = mix(uWarm, uCore, clamp(band*1.15, 0.0, 1.0));
    col *= band; float plume = smoothstep(0.0, 1.0, band) * 0.45;
    col += uWarm * plume * 0.35; col *= inner;
    col = 1.0 - exp(-col * uExposure);
    gl_FragColor = vec4(col, clamp(band, 0.0, 1.0));
  }`
);

const RingGlowMaterial = shaderMaterial(
  { uTime: 0, uRadius: 1.06, uSoft: 0.55, uColor: new THREE.Color("#ffb168").toArray() },
  `varying vec2 vUv; void main(){ vUv=uv*2.0-1.0; gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0); }`,
...
function StarField({ count = 1400, radius = 45 }) {
...
function EventHorizon({ radius = 0.88 }) {
  return <mesh><sphereGeometry args={[radius, 80, 80]} /><meshStandardMaterial color="#0a0b12" metalness={0.9} roughness={0.6} /></mesh>;
}

function AccretionDisk({ scale=[5.4,5.4,1], tilt=-0.22 }) {
...
function RingGlow({ scale=[5.9,5.9,1], tilt=-0.22 }) {
  const ref = useRef(); useFrame((_,dt)=>{ if(ref.current) ref.current.uTime += dt; });
  return (
    <mesh rotation={[tilt,0,0]} scale={scale}>
      <planeGeometry args={[2.6,2.6,1,1]} />
...
function Effects() {
  const { gl } = useThree();
  if (!gl?.capabilities?.isWebGL2) return null;
  return (
    <EffectComposer multisampling={2}>
      <Bloom intensity={0.75} luminanceThreshold={0.12} luminanceSmoothing={0.55} mipmapBlur />
      <Vignette eskil={false} offset={0.35} darkness={0.9} />
    </EffectComposer>
  );
}
...
function Scene({ centerPct=[0.62,0.35] }) {
  ...
  useFrame(()=>{
    ...
    boost.current = THREE.MathUtils.clamp(1 - d/0.25, 0, 1) * (isWeak ? 0.25 : 0.45);
  });
  ...
  useFrame((_,dt)=>{
    ...
    if (starsRef.current) starsRef.current.rotation.y += 0.005 * dt;
  });
  ...
  return (
    <>
      ...
      <group ref={starsRef}><StarField count={isWeak ? 700 : 1400} radius={45} /></group>
